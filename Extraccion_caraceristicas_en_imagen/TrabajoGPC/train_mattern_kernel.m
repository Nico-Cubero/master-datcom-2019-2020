...
ell = 1.9; % Escala inicial del núcleo mattern
sf = 1.0; % Varianza inicial del núcleo mattern
d = 3;
...
for i=1:5
    ...
    initial_hyp.cov = log([ell, ell, sf]);
    initial_hyp.d = 3;
    
    % Ajustar los hiperparámetros, ajustar el modelo y predecir el test
    prob = fit_predict_gp_models(train_healthy, train_malign, test_healthy, test_malign, 1, -1, 4, initial_hyp, @infVB, @meanZero, @covMaterniso, @likLogistic, -40);
    ...
    y_test = [repmat(1, size(test_healthy,1), 1);
                       repmat(-1, size(test_malign,1), 1)];
