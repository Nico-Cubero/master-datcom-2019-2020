...
object Script_HME_BD_DecissionTree {
       def main(arg: Array[String]) {
      
      //Configuración básica
      val jobName = "DecissionTree con HME_BD"
       ...
       ************************/
      if (arg.length != 6) {
        System.err.println("Especificar: <Método de evaluación de impureza> <Profundidad máxima del árbol> <Número máximo de bins> <Numero de árboles para el filtro de ruido> <Número máximo de particiones del filtro de ruido> <Máxima profundidad del ensembler del filtro de ruido>")
        System.exit(-1)
      }
      ...
      val numTrees_noise = arg(3).toInt
      val partitions_noise = arg(4).toInt
      val maxDepth_noise = arg(5).toInt
      ...
      val modelDT = DecisionTree.trainClassifier(trainClean, numClasses, categoricalFeaturesInfo, impurity, maxDepth, maxBins)
      ...
      **************************/
      ...
      //para cluster
      val writer = new PrintWriter(s"/home/x20620439/results_DecissionTree_impurity=$impurity-maxdepth=$maxDepth-max_Bins=$maxBins-HME_BD.txt")
      writer.write(
        s"Métricas obtenidas sobre DecissionTree con métrica de impureza '$impurity', profunidad máxima $maxDepth y número de bins máximos $maxBins\n" +
