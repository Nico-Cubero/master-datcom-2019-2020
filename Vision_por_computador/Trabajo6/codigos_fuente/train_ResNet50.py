...
Descripción: Entrena una red CNN partiendo del modelo ResNet50 preentrenado
            para clasificar imágenes en las que se muestre un peatón de aquellas
            en las que sólo aparezca fondo.
Uso: train_ResNet50.py -d <Documento JSON de experimentación> [-s]
...
from utils import (load_image_class_dataset, compute_HOG, train_svm, test_svm,
                    ...
                    no_pederestian_img_prepro, model_transfer_ResNet50, plot_results)
...
parser = argparse.ArgumentParser(description='Entrena un red CNN partiendo de'\
                                ' la red ResNet50 para identificar imágenes con '\
...
with open(exp_filename) as f:
    try:
        ...
        print('Se produjo un error con el fichero de experimentación expecificado:\n', str(e), file=sys.stderr)
        exit(-1)
...
for p in params:
    ...
    model = model_transfer_ResNet50(p['arquitectura'], p['load_weights'])[0]
