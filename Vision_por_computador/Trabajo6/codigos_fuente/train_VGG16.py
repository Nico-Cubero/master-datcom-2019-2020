...
Descripción: Entrena una red CNN partiendo del modelo VGG16 preentrenado
            para clasificar imágenes en las que se muestre un peatón de aquellas
            en las que sólo aparezca fondo.
Uso: train_VGG16.py -d <Documento JSON de experimentación> [-s]
...
from utils import (load_image_class_dataset, compute_HOG, train_svm, test_svm,
                    ...
                    no_pederestian_img_prepro, model_transfer_vgg16, plot_results)
...
parser = argparse.ArgumentParser(description='Entrena un red CNN partiendo de'\
                                 ' la red VGG16 para identificar imágenes con '\
...
for p in params:
    ...
    model = model_transfer_vgg16(p['arquitectura'], p['load_weights'])[0]
