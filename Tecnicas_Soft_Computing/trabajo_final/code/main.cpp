...
void save_best_results(const std::vector<MDPSolution> & sols,
												const std::vector<double> & n_iters,
												const std::vector<double> & n_evals,
...
int main(int argc, char ** argv) {
	...
	if (argc == 1) {
		...
		if (pop_size <= 0) {
			std::cout << "Size of population must be greater than 0" << std::endl;
			return -1;
		}

		if (arg_parser.n_contestants <= 0) {
			std::cout << "Number of contestants must be greater then 0" << std::endl;
			return -1;
		}

		if (arg_parser.p_cross < 0 || arg_parser.p_cross > 1) {
			std::cout << "Probability of crossovering must be in [0,1]" << std::endl;
			return -1;
		}

		if (arg_parser.p_mut < 0 || arg_parser.p_mut > 1) {
			std::cout << "Probability of mutation must be in [0,1]" << std::endl;
			return -1;
		}

		if (arg_parser.max_seconds <= 0) {
			std::cout << "Maximum number of executions must be greater than 0" << std::endl;
			return -1;
		}
		...
		if (dot_pos != std::string::npos) {
			out_dirname = inst_filename.substr(0, dot_pos) + "_results_r=" +
				std::to_string(n_executions) + "_p=" + std::to_string(pop_size) + "_k=" +
				std::to_string(arg_parser.n_contestants) + "_p-cross=" +
				std::to_string(arg_parser.p_cross) + "_p-mut=" +
				std::to_string(arg_parser.p_mut) + "_max_seconds=" +
				std::to_string(arg_parser.max_seconds);
		}
		else {
			out_dirname = inst_filename + "_results_r=" +
				std::to_string(n_executions) + "_p=" + std::to_string(pop_size) + "_k=" +
				std::to_string(arg_parser.n_contestants) + "_p-cross=" +
				std::to_string(arg_parser.p_cross) + "_p-mut=" +
				std::to_string(arg_parser.p_mut) + "_max_seconds=" +
	...
	std::vector<double> n_iterations;
	std::vector<double> n_evaluations;
	...
	for (size_t i=0; i<n_executions; i++) {
		...
		n_iterations.push_back(evolutionary.getNumIters());
		n_evaluations.push_back(evolutionary.getNumEvals());
	...
	save_best_results(bestSolutions, n_iterations, n_evaluations,
																out_dirname + '/' + "summary_results.txt");
...
void save_best_results(const std::vector<MDPSolution> & sols,
		...
		double avg_iters = mean(n_iters);
		double avg_evals = mean(n_evals);
		...
		f << "Average of iterations performed: " << avg_iters <<
		" - Avegare of evaluations performed: " << avg_evals << std::endl;
